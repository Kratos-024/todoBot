...
import getPdf from "./internetArchieve.controller";
...
const getAnswer = asyncHandler(async (req: Request, res: Response) => {
  ...
  5. If the user wants a **PDF of a book** (e.g., "Give me a PDF of this book" or "Can I get the PDF of <bookname> by <author>?"), extract the book name and author from the user's input and respond with a query to the Open Library API:
     { "query": "6", "bookName": "<bookname>", "author": "<author>", "url": "https://openlibrary.org/search.json?title=<bookname>&author=<author>" }
...
`;
  ...
  try {
    ...
    if (responsedObj["query"] == "1") {
      ...
      response = await getPdf(responsedObj["url"]);
    }
...
// export const formatThePdfUrl = async (
//   pdfData:any
// ) => {
//   const fullQ = `
//   You are an intelligent assistant. Based on the user input, identify what the user wants and respond in one of the following structured JSON formats:
